...
| --- | --- | --- | --- | --- |
| FreeRDP / WinPR | 当前源码树 `harmony/third_party/FreeRDP`，当前提交 `d00af99d5d6a` | Apache-2.0 | `libfreerdp3.so`, `libfreerdp-client3.so`, `libwinpr3.so`, `libfreerdp_ohos_probe.so` | 本仓库子模块源码，脚本 `harmony/scripts/wsl/build-freerdp-ohos.sh` 交叉编译 |
...
- 架构：`arm64-v8a`。
- 已关闭：`smartcard`, `PCSC`, `smartcard-pcsc`, `TSMF`, `CUPS`, `FUSE`, `OpenSLES`。
- 已启用：FreeRDP core、TLS/NLA、OpenSSL、zlib、cJSON、uriparser、RDPGFX、FFmpeg、OpenH264、OHOS AVCodec、OHOS Pasteboard、OHAudio。
...
4. 修改说明：FreeRDP OHOS client helper、证书策略、剪贴板、音频、RDPGFX/AVC444 GPU compositor、smartcard/TSMF 裁剪都要列入差异说明。
