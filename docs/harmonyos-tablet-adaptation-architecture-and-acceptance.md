...
> 专属权限不能继续物理存在于 tablet HAP。本文的 UI、响应式布局、RDP 会话、输入、Surface
> 和既有真机证据继续有效；第 2.1 至 2.3 节及其他“单 HAP 是固定发布边界”的表述只作为历史
> 决策与现状证据，不再指导后续打包实现。
...
2026-09-10（CHG-20260910-001，Implemented，Native/ArkTS检查Verified，真机待验收）：顶部会话工具栏已接入远端固定/跟随分辨率与重绘；局部叠放不改变Surface，48vp控件、hover与点击入口。分辨率过渡释放并屏蔽新指针输入，成功呈现或超时/失败恢复。组件由Index注入回调，未直接依赖Native。实际文件、测试命令/日志及A-D验收见专项调研实施回写，暂不实现任意本地缩放。

最新方向（2026-09-07，CHG-20260907-003）：用户要求优先调整远端分辨率，交互改为顶部hover/触屏点击展开工具栏。窗口跟随/固定模式、切换后实际画面生效确认与输入同步为首版；完整本地缩放变换后置。详见专项调研文首修订，尚未实施。

2026-09-07（CHG-20260907-002）：会话本地缩放与悬浮功能球已完成源码和同类产品调研，详细建议见 [专项调研](session-zoom-floating-controls-research.md)。拟采用Native统一视图变换、ArkTS功能球入口，保持本地vp、Surface物理像素和远端DPI分离；尚未实施，不修改本节既有验收状态。
