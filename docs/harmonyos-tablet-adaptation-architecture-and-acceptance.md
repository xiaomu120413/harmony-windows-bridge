...
| 所有权与时序 | 新建 `DisplayOrientationTracker` 由 `EntryAbility` 创建和销毁，持有主窗口 Display ID，监听 display change 与 window displayIdChange，并在内容加载后立即发布初值。Native 的 `RdpSessionChannels` 是最后一次合法 RDP orientation 的唯一状态所有者；SurfaceChanged、session connected、display-control connected 和方向主动更新四条 resize 路径都读取同一状态，从而不依赖系统先回调旋转还是先回调 SurfaceChanged，也不存在初连/通道重连退回 landscape 的旁路 |
...
| 计划代码文件 | 新建 `harmony/app/entry/src/main/ets/rdp/DisplayOrientationPolicy.ets`、`DisplayOrientationTracker.ets`；修改 `entryability/EntryAbility.ets`、`cpp/types/libentry/Index.d.ts`、`cpp/napi/api_exports.cpp`、`cpp/napi/native_bridge_context.h/.cpp`、`cpp/session/rdp_session_core.h/.cpp`、`cpp/session/rdp_session_channels.h/.cpp`、ArkTS 纯策略测试及测试入口 |
...
| 计划代码文件 | 新建 `rdp/SessionKeyboardEnvironment.ets`、`rdp/SessionImePolicy.ets`及其测试；修改 `components/session/RdpSessionPage.ets`、`pages/Index.ets`、`entryability/EntryAbility.ets`（只启停 KeyboardEnvironment）、`cpp/types/libentry/Index.d.ts`、`cpp/napi/api_exports.cpp`、`input/xcomponent_input_registration.cpp` 和测试入口 |
