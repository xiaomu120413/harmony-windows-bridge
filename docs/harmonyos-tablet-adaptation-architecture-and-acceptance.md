...
| 所有权与时序 | ArkTS 不再监听 display change 或计算方向，只在主窗口创建及 `displayIdChange` 时把 windowId/displayId 交给 Native。新建 `RdpDisplayOrientationMonitor` 注册 `OH_NativeDisplayManager_RegisterDisplayChangeListener()`，读取活动 Display 并更新 `RdpSessionChannels` 唯一 orientation；SurfaceChanged、session connected、display-control connected 和 Native display change 四条 resize 路径读取同一状态。XComponent 四向 Surface 变化仍由系统负责，应用不控制或重建它 |
| N-API/Native 边界 | 用薄接口 `configureHostWindow({windowId, displayId})` 代替 ArkTS `setDisplayOrientation()`；N-API 只传主窗口身份。方向监听、枚举映射、合法性、last-known 状态和主动 resize 全在 Native，结构化日志记录 displayId、native orientation、requested/sent RDP orientation |
...
| 交互与焦点 | 会话页新增唯一、明确的键盘开关；不创建 ArkTS `TextInput` 输入宿主。开启时先释放活动远端按键，再由 Native IME client attach/show，XComponent 焦点不转移；关闭、页面退出或会话断开时 Native hide/detach 并清理 preview。XComponent 普通触摸不自动弹键盘，因此 Native 取消无条件 `SetNeedSoftKeyboard(true)` |
...
| 键盘避让 | 进入会话保存当前 `KeyboardAvoidMode` 并设置 `NONE`，离开时恢复；新建由 `EntryAbility` 唯一启停的 `SessionKeyboardEnvironment` 监听主窗口 `keyboardHeightChange`，只把高度镜像到一个 AppStorage key，SessionPage 用它把键盘开关/输入状态提示移到键盘上沿。该高度不进入 Native，不改变 XComponent width/height，不触发远端 resize。远端 caret 位置不可由当前 RDP 通道获得，因此不伪造自动滚动或远端光标跟随 |
| N-API/Native 边界 | N-API 只暴露 `openRemoteIme()` / `closeRemoteIme()` 和公共的 `configureHostWindow()`，不暴露逐字符 `sendCommittedText` / `sendPlatformKey` 给 ArkTS。IME attach/proxy、预编辑、提交、删除、Enter 和状态清理由 Native `RemoteImeClient` 持有；ArkTS 只持有键盘按钮展示、`KeyboardAvoidMode.NONE` 和 keyboardHeight 避让。普通 XComponent 注册明确设置 `SetNeedSoftKeyboard(false)` |
| 计划代码文件 | 新建 `cpp/input/remote_ime_client.h/.cpp`、`rdp/SessionKeyboardEnvironment.ets`；修改 `components/session/RdpSessionPage.ets`（只有按钮和避让，无 TextInput）、`entryability/EntryAbility.ets`、`cpp/types/libentry/Index.d.ts`、`cpp/napi/api_exports.cpp`、`cpp/napi/native_bridge_context.h/.cpp`、`input/xcomponent_input_registration.cpp`、`cpp/CMakeLists.txt`；删除 ArkTS `SessionImePolicy` 及测试 |
