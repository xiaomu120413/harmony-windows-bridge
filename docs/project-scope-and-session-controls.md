...
## SESSION-ZOOM-001 — ResearchComplete / Proposed（尚未实现）

2026-09-07：用户确认问题为鸿蒙连接Windows时画面过大，要求先调研功能球。已核对当前等比适配、远端DPI请求、四条渲染路径和输入映射，建议会话内功能球提供缩小/放大、适应窗口、原始比例及显式移动画面模式。具体设计、文件落点和验收见 [悬浮功能球与缩放调研](session-zoom-floating-controls-research.md)。本轮仅调研，不修改功能代码；过大的现场根因尚待尺寸日志确认。

最新方向（CHG-20260907-003）：按用户反馈优先支持远端分辨率切换，入口改为顶部hover/点击展开工具栏，不限定功能球。提供固定/跟随窗口、应用与实际尺寸反馈；调整成功自动更新画面，hover只展开。手动本地缩放和平移降为后续阶段，不能与远端分辨率混为同一控件。详细状态机和文件落点见专项调研文首修订。
