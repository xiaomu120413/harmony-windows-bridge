...
| Change ID | 提交 | 类型/范围 | 改了什么 | 验证/关联 |
| --- | --- | --- | --- | --- |
| `CHG-20260916-002` | `revert: undo audit changes ca7d6ef on separate branch` | 审计整改回退 | 在codex/revert-ca7d6ef完整撤销ca7d6ef，保留main与历史；恢复此前会话及渲染代码，保留原顶部工具栏 | 反向补丁无冲突，与父节点代码树一致，diff检查通过；未重新构建/真机测试；[回退记录](revert-ca7d6ef.md) |
