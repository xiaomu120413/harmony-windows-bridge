...
| Change ID | 提交 | 类型/范围 | 改了什么 | 验证/关联 |
| --- | --- | --- | --- | --- |
| `CHG-20260916-001` | `security: rewrite git history to remove leaked signing material (M1)` | 仓库安全 | M1 步骤5：`git filter-repo --path tools/app/material --invert-paths --force` 重写 484 提交移除 material + `git push --force origin main`（`e3a85d7→e5c83ba`）。material 从公开 GitHub 历史移除。force push 前备份 material + stash；push 后从 stash index commit（9edf3024）恢复 22 个 untracked 新文件 + 从备份恢复本地 material。 | `build_app` debug 15.38s 0 error signed（恢复后代码完整）；force push 成功（forced update）；远程 main=e5c83ba 无 material；GitHub fork/缓存需联系 Support 清除；backup-m1-filterrepo 分支建议删除。[方案](signing-material-removal-plan.md) |

## 2026-09-15

| Change ID | 提交 | 类型/范围 | 改了什么 | 验证/关联 |
| --- | --- | --- | --- | --- |
| `CHG-20260915-001` | `docs: assess native_bridge_context god object (M5)` | Native/评估 | M5 评估：god object 被夸大，493 行在限制内，文档批准连线逻辑留此；真正违反点（全局变量+149行调度）迁移风险高收益微，建议保留现状。 | [评估](native-bridge-context-assessment.md)；未执行代码改动。 |
| `CHG-20260915-002` | `test: add RemoteControlCoordinator and RdpClientController.isConnectedState pure unit tests (H3)` | ArkTS/测试 | 新增 `RemoteControlCoordinator.test.ets`（16 用例：toStatus/displayMessage/applyStatus/applyPermissionResult/generateAccessCode/snapshot/ensureStarted 等）+ `RdpClientController.test.ets`（isConnectedState 2 用例），注册到 List.test.ets。纯 TS fake/spy，无 hamock。 | `hvigor test --mode module -p module=common@default` BUILD SUCCESSFUL 21s，30 测试通过（14 现有+新增）0 失败 0 错误；`RdpPermissionRequestCoordinator` routes 私有未测（需 hamock 或改可见性）。 |
| `CHG-20260915-003` | `refactor(session): extract RdpSessionStateCoordinator from Index and unlock H3 state machine tests` | 鸿蒙客户端/会话/测试 | H3：从 `Index.ets`（950→783 行，净减 167 行）提取连接/会话状态机到 `RdpSessionStateCoordinator.ets`（346 行），复刻 RemoteControlCoordinator 范式（构造注入+snapshot+onChange）。逐字搬移 onState/onError/startNativeConnect 决策树到 public handleState/handleError/handleConnectResult/connect，产出写内部态+publish，不调 ArkUI。Index 新增 applySessionStateSnapshot 落地 @State + 边沿触发（sessionVisible 上升沿→focusRemoteSurface，下降沿+isConnected→releaseActiveInput）。新增 `RdpSessionStateCoordinator.test.ets`（312 行，16 用例：snapshot 初始态/Resolving/Connected 首连 SUCCESS+persistProfile/RemoteLoginWaiting/RemoteDesktopReady/Failed CREDENTIALS+清 pending/Disconnected+pending NO_SESSION/Idle+pending NO_SESSION/onError danger+releaseInput/connect 校验失败+成功/handleConnectResult ok+!ok/register 幂等+失败/clearConnectionFeedback）。 | `hvigorw assembleHap` debug/hap/default BUILD SUCCESSFUL 8.3s 0 error signed（entry+entry_tablet）；`hvigorw test --mode module -p module=common@default` BUILD SUCCESSFUL 6.1s 46 用例通过（30 现有+16 新增）0 失败 0 错误；真机 connect→connected→disconnect + connect→failed 冒烟未执行。[设计](session-state-coordinator-design.md)、[台账 TAB-A-09](harmonyos-tablet-adaptation-architecture-and-acceptance.md) |

## 2026-09-14

| Change ID | 提交 | 类型/范围 | 改了什么 | 验证/关联 |
| --- | --- | --- | --- | --- |
| `CHG-20260914-005` | `refactor(gpu): split avc420/avc444 compositor internals and extract shared utils` | Native/渲染管线 | H4+M11：抽 `avc_gpu_common.h/.cpp`（逐字重复的常量/TimingBucket/ScopedTiming/NowMicros/H264解析）；avc420_internal(2509行)→5文件(utils160/decoder334/renderer858/state981/impl78)、avc444_internal(2593行)→5文件(utils287/decoder465/renderer955/readback92/state692)，每文件<1000行。不抽类基类（差异大+无真机）。渲染逻辑不变（inline→out-of-line 语义等价）。 | 阶段1 抽公共工具 build 8.23s；阶段2 拆 avc420 build 6s 0 error signed；阶段3 拆 avc444 build 6.02s 0 error signed；真机 AVC420/AVC444 渲染留验收。[设计](gpu-compositor-split-design.md)、[台账 TAB-A-08](harmonyos-tablet-adaptation-architecture-and-acceptance.md) | | `fix(native): harden freerdp session teardown and xrdp fork-exec safety` | Native/会话/xrdp | M7：`ohosSessionFree` 前置 `sessionCallbacks.userData=nullptr`，迟到回调走 null 分支防潜在 UAF；M8：xrdp fork 前构造独立 `envp`，移除子进程 `setenv`（违反 POSIX async-signal-safety），`execve` 改用 `envp.data()`；持锁 `usleep(150000)` 加注释说明。M10：核实 `max_desktop_width=0` 经 OHOS 后端 `ohos_desktop_size.c` 确认=无限制，无需改动。 | `build_app` debug/hap/default 8.62s `sign_status=signed` `error_count=0`；真机未执行。 |
| `CHG-20260914-003` | `fix(build): add release signing config and clean local credentials` | 签名/配置 | `build-profile.json5` 新增 release signingConfig（复用 `muhub.p12` 加密密码 + release p7b/cer）；`build_hap.bat` 加 `BUILD_MODE` 参数（默认 debug，支持 release）；`config.local.json` 清空真实内网 IP 和账号改占位。 | `build_app` debug/hap/default 22.4s `sign_status=signed` `error_count=0`；release 构建与真机未执行。 |
| `CHG-20260914-001` | `fix(session): add active disconnect channel and unify connection state source` | 鸿蒙客户端/Native/会话 | 新增非阻塞 `disconnect` NAPI 通道，转发 native 已有 `RdpSession::RequestDisconnect()`；`RdpSessionPage`/`MuHubApp` 销毁时主动断连，`onPageHide` 不断；删除 `RdpClientController.connect()` 同步状态写入，`connected` 改由 `onState`/`onError` 单一驱动。缓解连上断不开与状态显示错乱。 | Native/ArkTS 链路检查通过；`build_app` debug/hap/default 18.15s `sign_status=signed` `error_count=0`；真机断连/重连/切后台回归未执行；[实施台账 TAB-A-07](harmonyos-tablet-adaptation-architecture-and-acceptance.md)、[验收基线](freerdp-ohos-validation-baseline.md) |
| `CHG-20260914-002` | `docs: make latest_frame_renderer GDI buffer lifetime contract explicit` | Native/渲染管线 | `latest_frame_renderer.h` 接口、`freerdp_gdi_bridge.cpp` 两条 free/resize 路径旁、`ohos-native-cpp-module-guidelines.md` 显式化「释放 `gdi->primary_buffer` 前必须先 `StopGdiRenderPipeline` join worker」的生存期契约。当前 `HarmonyPostDisconnect`/`HarmonyDesktopResize` 已遵守，无 active UAF；防止未来新增路径遗漏导致 UAF。 | 文档/注释改动，无运行时行为变化，未执行构建（注释不影响编译）。 |
...
| Change ID | 提交 | 类型/范围 | 改了什么 | 验证/关联 |
| --- | --- | --- | --- | --- |
| CHG-20260907-003 | docs: prioritize resolution controls in top toolbar | 调研修订/会话显示 | 按用户反馈改为顶部hover/点击工具栏及远端分辨率切换优先；补充固定/跟随冲突、实际画面确认、刷新边界和失败反馈。 | 复核已有resize状态、coordinator和官方交互资料；仅更新设计文档，未实现或构建。 |
| CHG-20260907-002 | docs: research session zoom and floating controls | 调研/会话显示 | 针对鸿蒙连接Windows画面过大，核对本地适配与远端DPI链路，提出功能球、缩放/平移统一变换、输入隔离和分阶段验收方案。 | 本地源码、AnyDesk/Windows App官方资料及平台文档核对；新增专项调研并同步范围/索引/适配方案。仅文档，无代码修改、构建或新增真机测试。 |
| CHG-20260907-001 | docs: mark split packaging user verified | 文档/分包验收 | 按用户测试反馈，将上次交付的HAP/HSP/HNP分包方式标记为User Verified，作为后续维护基线；同步打包方案与验证基线。 | 用户确认“已经测试了，上次的分包方式没问题”；关联CHG-20260905-006交付记录。仅更新文档，未新增构建或自动真机测试。 |
