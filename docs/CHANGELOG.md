...
| Change ID | 提交 | 类型/范围 | 改了什么 | 验证/关联 |
| --- | --- | --- | --- | --- |
| `CHG-20260812-001` | `docs(rdp): verify input and HNP lifecycle` | 文档/输入、IME、xrdp HNP 生命周期 | 根据用户真机确认，将输入与 IME 专项 `TAB-F-01`～`TAB-F-06` 及 2in1 HNP 生命周期 `MDP-03B`/`HNP-01`～`HNP-05` 更新为 Verified；明确不改变 TAB-F-07 权限通道、外设故障矩阵和 tablet Entry 分发迁移状态。 | 用户 2026-08-12 真机确认；专项台账与故障验收清单交叉核对，`git diff --check` 通过。 |
